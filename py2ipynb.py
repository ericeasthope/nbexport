...
def parsePy(py_filename, cellmark_style, other_ignores=[]):
    ...
    with open(py_filename, "r") as f:
        ...
        for l in f:
            ...
            if lines and (
                (l1.startswith("# In[") and l1.endswith("]:"))
                or l1 == CELLMARKS[cellmark_style]
            ):
                yield (codecell, metadata, "".join(lines).strip(linesep))
                lines = []
                codecell = True
            ...
            if (codecell and l1 not in codecell_ignores) or (
                not codecell and l1 not in nocodecell_ignores
            ):
                lines.append(l)
...
if __name__ == "__main__":
    ...
    if False:  # args.cellmark_style == "default":
...
def py2ipynb(readIn):
    ...
    """
    # write out images, !! personalize !!, ! make '/img' if not found !
    for relPath, imgData in resources['outputs'].items():
        imgName = relPath.split('/')[-1] # get filename
        imgPath = '{}/{}'.format(IMAGE_DIR, imgName) # build newpath
