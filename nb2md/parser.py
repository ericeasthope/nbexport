...
NOTEBOOK_DIRECTORY = os.path.join(MODULE_DIRECTORY, ".")
MARKDOWN_DIRECTORY = os.path.join(MODULE_DIRECTORY, "tmp")
            ...
            for path in post_paths:
                if path.split("-")[-1].split(".")[0] == name:
                    ...
                    markdown_path = os.path.join(
                        MARKDOWN_DIRECTORY,
                        notebook["date_modified"] + "-" + name + ".md",
                    )

            if len(post_paths) == 0:
                markdown_path = os.path.join(
                    MARKDOWN_DIRECTORY, notebook["date_created"] + "-" + name + ".md"
                )
    ...
    def _add_front_matter(self, name, notebook, markdown_file):
        ...
        for h in header:
            if h.startswith("[//]"):
                ...
                front_matter["tags"] = hash_tags.replace("#", "").split()
            elif h.startswith("# "):
        ...
        for name, notebook in zip(self.names, self.notebooks):
            for cell in notebook["notebook"]["cells"]:
                ...
                if source.startswith("![](") and source.endswith(")"):
                    ...
                    source_out = "![](" + "{{ site.url }}/images/" + image_name + ")"
                    cell["source"] = source_out
